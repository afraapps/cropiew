...
import ir.afraapps.widget.cropiew.CropOverlayView.CropWindowChangeListener
        ...
        if (state is Bundle) {
            ...
            if (bitmapLoadingWorkerJob == null && imageUri == null && originalBitmap == null && mImageResource == 0) {
                var uri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    state.getParcelable("LOADED_IMAGE_URI", Uri::class.java)
                } else {
                    @Suppress("DEPRECATION")
                    state.getParcelable("LOADED_IMAGE_URI")
                }
                if (uri != null) {
                    ...
                    if (resId > 0) imageResource = resId
                        ...
                        uri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                            state.getParcelable("LOADING_IMAGE_URI", Uri::class.java)
                        } else {
                            @Suppress("DEPRECATION")
                            state.getParcelable("LOADING_IMAGE_URI")
                        }
                ...
                mCropOverlayView.setCropShape(CropShape.valueOf(state.getString("CROP_SHAPE")!!))
 ...
 * Large Bitmaps Efficiently](http://developer.android.com/training/displaying-bitmaps/load-bitmap.html).
    ...
    init {
        ...
        mCropOverlayView = CropOverlayView(context).apply {
